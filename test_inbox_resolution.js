...
async function runAllTests() {
  ...
  test('R1: getPhoneVariants produces clean digits and standard formats', () => {
    ...
    // International dial prefix 00 (e.g. 0061400040043)
    const v00 = inbox.getPhoneVariants('0061400040043');
    assert(v00.includes('61400040043'), 'Must convert international 00 prefix to standard digits');
    assert(v00.includes('+61400040043'), 'Must produce +E164 from 00 prefix');
    assert(v00.includes('0400040043'), 'Must produce AU local format from 0061 prefix');
    ...
    // Edge cases: null, undefined, empty, spaces, plus only, pure non-digits
    ...
    assert.deepStrictEqual(inbox.getPhoneVariants('+'), []);
    assert.deepStrictEqual(inbox.getPhoneVariants('+++'), []);
    assert.deepStrictEqual(inbox.getPhoneVariants('foo'), [], 'Non-digit string must return empty array');
    const vAlphanum = inbox.getPhoneVariants('foo123');
    assert(vAlphanum.includes('123'), 'Alphanumeric string must extract digits');
  });
  ...
  test('R1: toPostgrestInList quotes values containing special characters to prevent PostgREST syntax errors', () => {
    ...
    // Hardening: edge cases with null, undefined, empty strings, quote-only strings, whitespace, numbers
    const mixed = ['+61400040043', null, undefined, '', '   ', '""', '"   "', '"', '"""', 61400040043];
    const cleanList = inbox.toPostgrestInList(mixed);
    assert.strictEqual(cleanList.length, 2, 'Must filter out null, undefined, whitespace, and quote-only strings');
    ...
    const query = dummyClient.from('messages').select('dir,body').in('phone', cleanList);
    ...
    assert(!searchString.includes('%2C%2C'), 'Query string MUST NOT contain empty items or consecutive commas');
  });
  ...
  await asyncTest('R1: Bokun booking retrieval handles phone normalization', async () => {
    ...
    const b4 = await bokun.getBookingsForPhone('0061411222333');
    assert(b4.length >= 1, 'Should find booking by international 00 prefix');

    const bEmpty = await bokun.getBookingsForPhone('');
    assert.deepStrictEqual(bEmpty, []);

    const bInvalid = await bokun.getBookingsForPhone('foo');
    assert.deepStrictEqual(bInvalid, [], 'Should return empty array for non-digit phone');
  });
  ...
  test('R2 & R3: inbox.html contains required URL encoding, Call UX and status badges', () => {
    ...
    // Check sender tags in message bubbles
    assert(html.includes('class="sender-tag"'), 'Must render sender tag on message bubbles');
    assert(html.includes('data-sender'), 'Must identify bubble sender via data-sender');

    // Check scroll preservation on background poll
    assert(html.includes('wasNearBottom'), 'Must check if user was near bottom before scrolling');
    assert(html.includes('prevScrollTop'), 'Must preserve previous scroll position when user is reading history');

    // Check tab visibility and idle-aware backoff polling
    assert(html.includes('document.hidden'), 'Must pause polling when tab is backgrounded');
    assert(html.includes('visibilitychange'), 'Must listen to visibilitychange for fast resume');
    assert(html.includes('getPollIntervalMs'), 'Must dynamically compute polling interval with idle backoff');

    // Check chat reset on close / back button
    assert(html.includes('closeChat'), 'Must implement closeChat to cleanly reset states on back navigation');
    assert(html.includes("d.startsWith('00')"), 'formatE164 must handle 00 international prefix');
  });
